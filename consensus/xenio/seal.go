...
/*func (ethash *Xenio) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
...
// stake is the actual proof-of-work miner that searches for a nonce starting from
// seed that results in correct final block difficulty.
/*
func (ethash *Xenio) stake(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	...
	logger := log.New("staker", id)
	logger.Trace("Started staking with 13 eyes", "stake", seed)
	for {
		select {
		case <-abort:
			// Staking terminated, update stats and abort
			logger.Trace("staking operation aborted", "attempts", nonce-seed)
			return
